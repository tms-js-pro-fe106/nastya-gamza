...
const getUniqMovies = (movies: Movie[]): string[] => {
...
const getUniqActors = (movies: Movie[]): string[] => {
    return  Array.from(new Set(movies.flatMap(movie => movie.actors)));
};
console.log(getUniqActors(movies));


// 3. Отсортировать фильмы по рейтингу по убыванию
const sortMovies = (movies: Movie[]): Movie[] => [...movies].sort((a, b) => b.imdbRating - a.imdbRating)
...
const createNewMovie = (movies: Movie[]): newMovie[] => movies.map(({ id, title, released, plot }) => ({ id, title, released, plot }));
...
const filterMoviesByTitle = (movies: Movie[], title: string): Movie[] => {
...
const filterMoviesByTitleOrPlot = (movies: Movie[], word: string): Movie[] => {
  return movies.filter(movie => movie.title.toLowerCase().includes(word.toLowerCase()) || movie.plot.toLowerCase().includes(word.toLowerCase()));
}
...
const filterMoviesByField = (movies: Movie[], field: string, value: string | number): Movie[] => {
  return movies.filter(movie => movie[field as keyof Movie] === value);
}
