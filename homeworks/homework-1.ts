...
const getUsersWithAnimals = (users: User[]): User[] => users.filter((user) => user.animals && user.animals.length > 0);
...
function getCarNames(users: User[]): string {
  return users.reduce((acc: string[], user: User) => {
